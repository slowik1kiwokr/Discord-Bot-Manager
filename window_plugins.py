...
''',
    ...
    "Statisztika módosító": '''def setup_panel(panel):
    """Aktív bot nevének és verziójának módosítása."""
    panel.log_event("EVENT", "Statisztika módosító plugin aktív.")
    # Példa: ha Main Bot fut, akkor minden indítás után +1 hozzáadás
    # (Ez csak bemutató, ne futtasd élesben!)
    def example():
        bot = panel.bots.get(panel.active_bot_key)
        if bot:
            bot["total_commands"] += 1
            panel.lbl_commands.configure(text=str(bot["total_commands"]))
        panel.after(60000, example)
    panel.after(60000, example)
''',

    "Automatikus mentés induláskor": '''def setup_panel(panel):
    """Induláskor készít egy biztonsági mentést."""
    panel.log_event("EVENT", "Auto-backup plugin betöltve.")
    panel.after(10000, lambda: panel.create_backup(show_message=False))
''',
...
''',
    ...
    """Figyeli, ha a bot leáll, és naplózza."""
...
''',
    ...
    """Extra hangjelzés, ha a naplóban ERROR jelenik meg."""
    import winsound
...
''',
    ...
    "Időjárás lekérdező (API példa)": '''def setup_panel(panel):
    """Példa külső API hívásra (Open-Meteo, nem kell API kulcs)."""
    import customtkinter as ctk
    ...
    def open_weather():
        ...
        ctk.CTkLabel(win, text="🌤️ Időjárás (Budapest)",
                     font=("Arial", 16, "bold")).pack(pady=12)
        output = ctk.CTkTextbox(win, height=180, width=340)
        output.pack(padx=20, pady=8)
        output.insert("1.0", "Betöltés...")

        def fetch():
            try:
                url = ("https://api.open-meteo.com/v1/forecast"
                       "?latitude=47.5&longitude=19.04&current_weather=true")
                with urllib.request.urlopen(url, timeout=6) as r:
                    data = json.loads(r.read().decode())
                cw = data["current_weather"]
                text = (f"Hőmérséklet: {cw['temperature']} °C\\n"
                        f"Szél: {cw['windspeed']} km/h\\n"
                        f"Idő: {cw['time']}")
            except Exception as e:
                text = f"Hiba: {e}"
            win.after(0, lambda: (output.delete("1.0", "end"),
...
''',
    ...
    "Backup rotáció (régi törlése)": '''def setup_panel(panel):
    """Legfeljebb 10 mentést tart meg, a régieket törli."""
    import os

    def rotate():
        backup_dir = panel.backup_dir if hasattr(panel, "backup_dir") else None
        if not backup_dir:
            from modules.config import BACKUP_DIR as backup_dir
        try:
            files = sorted(
            ...
            for old in files[10:]:
                ...
                panel.log_event("EVENT", f"[ROTÁCIÓ] Törölve: {old}")
        except Exception as e:
            panel.log_event("ERROR", f"[ROTÁCIÓ] Hiba: {e}")

    panel.after(5000, rotate)
''',

    "Discord webhook értesítés": '''def setup_panel(panel):
    """Hiba esetén webhookra küld értesítést. Állítsd be a saját URL-t!"""
    import urllib.request
    import json
...
class WindowPluginsMixin:
    ...
    # --------------------------------------------------------------
    #  Plugin lista frissítése
    # --------------------------------------------------------------
    def _refresh_plugin_list(self, listbox):
    ...
    def open_plugins_window(self):
        ...
        header = ctk.CTkLabel(
            win, text="🧩 " + self.tr("plugins"),
            font=("Arial", 18, "bold"), text_color="#7f8c8d"
        )
        header.pack(pady=(12, 4))

        ctk.CTkLabel(
            win, text=self.tr("plugins_help"),
